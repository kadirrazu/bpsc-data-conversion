...
//Input DBF File
$dbf_file = __DIR__ . '\..\file-io\file-to-convert\for-result\tab_fin_bcs45.DBF';

//Output SQL-INSERT File
$sql_output_file = __DIR__ . '/../file-io/file-output/dbf-to-file/bcs-result/bcs45/candidates/candidates_sql_bcs45_with_gf_data.sql';

//Output PHP-ARRAY File
$php_array_output_file = __DIR__ . '/../file-io/file-output/dbf-to-file/bcs-result/bcs45/candidates/candidates_array_bcs45_with_gf_data.php';

//Encoding of DBF File
$encoding = 'CP1252';

//Table name
$table_name = "candidates_45_with_gf";
...
$field_map = [
    ...
    'choice_list_backup' => 'choice_list_backup',
    'TPASS_CODE' => 'choice_list_tech',
    'choice_list_tech_backup' => 'choice_list_tech_backup',
	'FIN_CHO' => 'gf_decision_text',
	'PCAD_NAME' => 'existing_job_cadre',
	'has_quota' => 'has_quota',
...
try {
	...
	//pass_code and t_pass_code backup
	foreach ($mapped as &$row) {
		if(empty($row['choice_list_backup'])) {
				$row['choice_list_backup'] = $row['choice_list'];
		}
	}
	...
	foreach ($mapped as &$row) {
		if(empty($row['choice_list_tech_backup'])) {
				$row['choice_list_tech_backup'] = $row['choice_list_tech'];
		}
	}
